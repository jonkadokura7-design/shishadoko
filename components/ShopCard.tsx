    ...
    return (
        <div className="glass-card rounded-xl p-6 mb-4 transform transition-all hover:scale-[1.02] hover:bg-white/5 border border-white/10 group">
            <div className="flex justify-between items-start mb-2">
                <h3 className="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-amber-100 to-amber-400 group-hover:from-amber-200 group-hover:to-amber-500">
                    {place.name}
                </h3>
                {place.opening_hours?.open_now !== undefined && (
                    <span className={`text-xs px-2 py-1 rounded-full border ${place.opening_hours.open_now ? 'border-green-500/50 text-green-400 bg-green-900/20' : 'border-red-500/50 text-red-400 bg-red-900/20'}`}>
